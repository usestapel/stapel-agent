...
| `STT_DOWNLOAD_ALLOWED_HOSTS` | `[]` | Exact-host allowlist for audio URLs, applied to every redirect hop. A deployment that only ever passes presigned URLs from its own object store lists that store here. **Empty is not a wildcard**: with no allowlist the download is refused unless `STT_DOWNLOAD_ALLOW_ANY_HOST` is on, and `stapel_agent.W015` says so at boot. An SSRF ceiling, so it is in `conf.NO_ENV` — an environment variable is ignored for it and it has to be stated in `settings.py`. |
